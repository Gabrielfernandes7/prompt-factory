...
export function createPromptLibrary({ search, domain, technique, results, preview, toast }) {
    let selected = null;
    const render = () => {
    ...
    const selectPrompt = id => {
        ...
        preview.innerHTML = `<div class="framework-card-head"><div><span class="section-label">${escapeHtml(selected.domain)} · ${escapeHtml(selected.subdomain)}</span><h3>${escapeHtml(selected.name)}</h3></div><button class="btn" id="copyLibraryPrompt" type="button">Copiar</button></div><p>${escapeHtml(selected.objective)}</p><div class="library-fields">${fields.map(field => `<label>${escapeHtml(field)}<input data-placeholder="${escapeHtml(field)}" placeholder="Preencha ${escapeHtml(field.toLowerCase())}"></label>`).join('')}</div><pre id="libraryOutput"></pre>`;
        const update = () => {
        ...
        preview.querySelector('#copyLibraryPrompt').addEventListener('click', async () => { await navigator.clipboard.writeText(preview.querySelector('#libraryOutput').textContent); toast('Template copiado.'); });
        update();
    };
    [search, domain, technique].forEach(control => control.addEventListener('input', render));
    render();
}
